...
def compute_ik(hand_id, target_positions, end_effector_indices):
    """
    Compute inverse kinematics for the hand using only fingertip positions.
    ...
    # Extract fingertip positions from target_positions
    ...
    print(f"Thumb tip pos: {thumb_tip_pos}, Index tip pos: {index_tip_pos}, Middle tip pos: {middle_tip_pos}, Ring tip pos: {ring_tip_pos}")
    
    # Find the joint indices for the tip_head links
    tip_head_indices = []
    for i in range(p.getNumJoints(hand_id)):
        ...
        if joint_name in ["thumb_tip", "index_tip", "middle_tip", "ring_tip"]:
            tip_head_indices.append(i)
            print(f"Found tip_head joint: {joint_name} at index {i}")
    
    # If we found all four tip_head joints, use them as end effectors
    if len(tip_head_indices) == 4:
        print("Using tip_head links as end effectors")
        # Try a different mapping:
        leap_end_effector_pos = [
            index_tip_pos,     # For index_tip_head
            middle_tip_pos,    # For middle_tip_head
            ring_tip_pos,      # For ring_tip_head
            thumb_tip_pos      # For thumb_tip_head
        ]

        # Calculate IK using tip_head links
        ...
        print("Using fingertip joints as end effectors")
        # Use only the fingertip joints (d, h, l, p) as end effectors
        fingertip_indices = [4, 9, 14, 19]  # Indices for d, h, l, p
        
        # Try a direct mapping:
        leap_end_effector_pos = [
            index_tip_pos,     # For joint index 4 (d) - Index fingertip
            middle_tip_pos,    # For joint index 9 (h) - Middle fingertip
            ring_tip_pos,      # For joint index 14 (l) - Ring fingertip
            thumb_tip_pos      # For joint index 19 (p) - Thumb fingertip
        ]
        
        # Calculate IK using fingertip joints
        joint_poses = p.calculateInverseKinematics2(
            hand_id,
            fingertip_indices,
            leap_end_effector_pos,
    ...
    # Create a mapping from letter to joint index
    letter_to_joint_idx = {}
    for i in range(p.getNumJoints(hand_id)):
        joint_info = p.getJointInfo(hand_id, i)
        joint_name = joint_info[1].decode('utf-8') if isinstance(joint_info[1], bytes) else str(joint_info[1])
        if joint_name in ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p']:
            letter_to_joint_idx[joint_name] = i
    
    # Create a mapping from letter to IK solution index
    letter_to_ik_idx = {
    ...
    if len(joint_poses) >= 16:
        ...
        # Reverse some joint pairs as in the example
...
def main():
    ...
    # Define end effector joint indices
    leap_end_effector_indices = [3, 4, 8, 9, 13, 14, 18, 19]
    print("\n=== End Effector Joints ===")
