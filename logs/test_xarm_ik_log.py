...
def convert_position_pybullet_to_openxr(position):
    """Convert a position vector from PyBullet (Z-up) to OpenXR (Y-up)."""
    x, y, z = position
    return np.array([x, z, y])  # Swap Y and Z

def convert_quaternion_pybullet_to_openxr(quaternion):
    """Convert a quaternion from PyBullet (Z-up) to OpenXR (Y-up)."""
    q_pybullet = Rotation.from_quat(quaternion)  # (x, y, z, w)
    q_x_90 = Rotation.from_euler('x', 90, degrees=True)  # 90° rotation around X
    q_openxr = q_x_90 * q_pybullet  # Apply rotation
    return q_openxr.as_quat()

def convert_position_openxr_to_pybullet(position):
    """Convert a position from OpenXR (Y-up) to PyBullet (Z-up)."""
    x, y, z = position
    return np.array([x, z, y])  # Swap Y and Z

def convert_quaternion_openxr_to_pybullet(quaternion):
    """Convert a quaternion from OpenXR (Y-up) to PyBullet (Z-up)."""
    q_pybullet = Rotation.from_quat(quaternion)  # (x, y, z, w)
    q_x_90 = Rotation.from_euler('x', -90, degrees=True)  # -90° rotation around X
    q_openxr = q_x_90 * q_pybullet  # Apply rotation
    return q_openxr.as_quat()

def test_xarm_ik(hm_series):
    # Connect to PyBullet
    ...
    end_effector_index = 7  # The arm end-effector link should be at index 6
    ...
    for hm in hm_series:
        ...
        try:
            ...
            target_pos = cart[:3]
            target_pos = convert_position_openxr_to_pybullet(target_pos)
            target_orn = cart[3:]
            target_orn = convert_quaternion_openxr_to_pybullet(target_orn)
            # print(f'\ncart: {cart}')
            ...
            last_current_state = p.getLinkState(robot_id, end_effector_index)
            ...
            while True:
                ...
                delta = np.linalg.norm(np.array(current_state[0] + current_state[1]) -
                                       np.array(last_current_state[0]+ last_current_state[1]))
                # Print debug info
...
if __name__ == "__main__":
    ...
    hm_series = [np.array(frame['transformed_pose']) for frame in frames_data]

    test_xarm_ik(hm_series)
