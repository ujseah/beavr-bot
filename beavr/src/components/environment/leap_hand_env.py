from beavr.src.components.environment.pybullet_base_env_hand import PyBulletBaseEnvHand
...
class LeapHandEnv(PyBulletBaseEnvHand):
    ...
    def compute_IK(self, hand_pos):
        ...
        if len(hand_pos) != 8:
            print(f"Error: Expected 8 positions but got {len(hand_pos)}")
            return None
        ...
        print("Combined joint poses:", combined_jointPoses)
        ...
        if hand_pos is not None:
            ...
            if action is not None:
                for i in range(20):
                    ...
                    print("Moving", LEAP_HAND_JOINTS[i], "to", action[i])
                # Step the simulation
                p.stepSimulation()
    ...
    def get_meta_quest_data(self):
        ...
        if raw_keypoints is None or len(raw_keypoints) < max(QUEST_JOINTS['pinky'])+1:
            print("Warning: Invalid or missing keypoint data from Meta Quest")
            return None
