from beavr.src.components.environment.pybullet_base_env import PyBulletBaseEnv
from beavr.src.utils.network import ZMQCompressedImageTransmitter
from beavr.src.utils.images import rescale_image
from beavr.src.constants import VIZ_PORT_OFFSET
...
from beavr.src.utils.logger import RobotLogger
...
class XArmEnv(PyBulletBaseEnv):
    def __init__(self, 
        ...
        print("Publishing initial position...")
        ...
        print(f"Arm joint indices: {self.movable_joint_indices}")
    ...
    def load_assets(self):
        ...
        print("Loading XArm assets...")  # Debug print
        current_dir = os.path.dirname(os.path.abspath(__file__))
        
        # Print existing objects before loading
        print(f"Objects before loading: {[p.getBodyInfo(i)[0].decode('utf-8') for i in range(p.getNumBodies())]}")
        ...
        print(f"Objects after loading: {[p.getBodyInfo(i)[0].decode('utf-8') for i in range(p.getNumBodies())]}")

    def reset(self):
        """Reset robot state."""
        print("Resetting XArm...")  # Debug print
    ...
    def cleanup(self):
        ...
        try:
            ...
            print("Successfully cleaned up XArmEnv resources")
