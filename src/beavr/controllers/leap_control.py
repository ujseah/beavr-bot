    ...
    def __init__(self):
        ...
        for motor_id in self.motor_ids:
            ...
            if not self.groupBulkReadPosition.addParam(motor_id, ADDR_PRESENT_POSITION, LEN_PRESENT_POSITION):
                print(f"[ID:{motor_id}] groupBulkRead addparam failed for position")
            # Velocity
            if not self.groupBulkReadVelocity.addParam(motor_id, ADDR_PRESENT_VELOCITY, LEN_PRESENT_VELOCITY):
                print(f"[ID:{motor_id}] groupBulkRead addparam failed for velocity")
            # Current
            if not self.groupBulkReadCurrent.addParam(motor_id, ADDR_PRESENT_CURRENT, LEN_PRESENT_CURRENT):
                print(f"[ID:{motor_id}] groupBulkRead addparam failed for current")
    ...
    def bulk_write_positions(self, positions):
        ...
        try:
            ...
            for i, motor_id in enumerate(self.motor_ids):
                ...
                if result != True:
                    print(f"Failed to add parameter for motor {motor_id}")
                    return False
            
            # Execute bulk write
            result = self.groupBulkWrite.txPacket()
            if result != COMM_SUCCESS:
                print(f"Failed to execute bulk write: {self.packetHandler.getTxRxResult(result)}")
                return False
        ...
        for attempt in range(max_retries):
            try:
                ...
                if comm_result != COMM_SUCCESS:
                    print(f"Bulk read failed (attempt {attempt + 1}/{max_retries}): {self.packetHandler.getTxRxResult(comm_result)}")
                    continue
                ...
                print(f"Bulk read error (attempt {attempt + 1}/{max_retries}): {e}")
...
class DexArmControl:
    ...
    def move_hand(self, target_joints_radians):
        ...
        try:
            ...
            print(f"Failed to queue movement: {e}")
            return False
