...
class RGBImageRecorder(Recorder):
    ...
    def stream(self):
        logger.info('Starting to record RGB frames from port: {}'.format(self._image_stream_port))
        ...
        # Storing the data
        logger.info('Storing the final version of the video...')
        self.recorder.release()
        store_pickle_data(self._metadata_filename, self.metadata)
        logger.info('Stored the video in {}.'.format(self._recorder_file_name))
        logger.info('Stored the metadata in {}.'.format(self._metadata_filename))
...
class DepthImageRecorder(Recorder):
    ...
    def stream(self):
        ...
        logger.info('Starting to record depth frames from port: {}'.format(self._image_stream_port))
        ...
        logger.info('Compressing depth data...')
        ...
        logger.info('Saved compressed depth data in {}.'.format(self._recorder_file_name))
...
class FishEyeImageRecorder(Recorder):
    def __init__(
        ...
        # Subscribing to the image stream port
        logger.info("Image Stream Port", image_stream_port)
        self._host, self._image_stream_port = host, image_stream_port
        self.image_subscriber = ZMQCameraSubscriber(
            host = host,
            port = image_stream_port,
