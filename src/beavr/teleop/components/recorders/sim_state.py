...
class SimInformationRecord(Recorder):
    ...
    def stream(self):
        logger.info('Checking if the keypoint port is active...')

        logger.info('Starting to record keypoints to store in {}.'.format(self._recorder_file_name))
            ...
            try:
                ...
                if self.robot=='allegro':
                    ...
                    if self.recorder_function_key =='joint_states':
                        ...
                        logger.info("Entering Commanded Cartesian States")
                    else:
                        commanded_joint_angles=self.subscriber.recv_keypoints()
                        logger.info("Entering Commanded Joint States")
                else:
                        ...
                        if self.recorder_function_key == 'joint_states':
                            if key not in self.robot_information.keys():
                                ...
                                if key =='timestamps':
                                    ...
                                    self.robot_information[key].append(actual_joint_angles)
                            logger.info('Joint angles recorded', actual_joint_angles)
        ...
        logger.info('Saved keypoint data in {}.'.format(self._recorder_file_name))