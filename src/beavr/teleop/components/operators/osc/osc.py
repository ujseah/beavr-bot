    ...
    def __init__(
        ...
        **kwargs,                   # hacky way to sink extraneous args
        ...
        for variable_gain, gain_limits, dim in zip(
            ...
            (6, 6, self.control_dim), strict=False,
        ):
            if variable_gain:
...
def create_osc_controller(device="cpu"):
    """Create OSC controller configured for xArm7 (7-DOF)."""
    controller = OSCController(
        # Input/output ranges for commands (dx, dy, dz, dax, day, daz)
        input_min=[-1.0] * 6,  # Normalized input range
        input_max=[1.0] * 6,
        output_min=[-0.1] * 6,  # Actual movement range (in meters/radians)
        output_max=[0.1] * 6,
        
        # Joint torque limits
        control_min=[-10.0] * 7,  # 7 joints
        control_max=[10.0] * 7,   # 7 joints
        
        control_noise=0.0,
        control_dim=7,  # Specify 7-DOF
        device=device,
        
        # Controller gains
        kp=150.0,  # Position gains
        kp_limits=(10.0, 300.0),
        damping_ratio=1.0,  # Damping
        damping_ratio_limits=(0.0, 2.0),
        kp_null=10.0,  # Null space control gains
        kp_null_limits=(0.0, 50.0),
        
        # Optional: Set rest position for null space control
        rest_qpos=[0.0] * 7,  # Default joint positions
        
        decouple_pos_ori=True  # Separate position and orientation control
    )
    return controller
