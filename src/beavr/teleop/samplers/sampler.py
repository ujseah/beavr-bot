...
class Sampler(ABC):
    ...
    def _get_image_frame_timestamps(self):
        ...
        if self.data_type == 'rgb' or self.data_type == 'all':
            logger.info('Obtaining all the RGB image timestamps')
        ...
        if self.data_type == 'depth' or self.data_type == 'all':
            logger.info('Obtaining all the Depth image timestamps')
    ...
    def save_sampled_rgb_frames(self, cam_idx, store_path):
        if self.data_type == 'rgb' or self.data_type == 'all':
            ...
            logger.info('Writing the frames.')
            ...
            logger.info('Storing {} frames'.format(num_frames_recorded))
            logger.info('Saving video in {}'.format(store_path))
            writer.release()
            capture.release()
