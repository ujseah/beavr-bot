...
class BaseLogger:
    ...
    def _cleanup_temp_files(self):
        ...
        if temp_files:
            print(f"Found {len(temp_files)} temporary log files. Cleaning up...")
                    ...
                    if os.path.exists(main_file):
                        try:
                            ...
                            if 'frames' in temp_data and 'frames' in main_data:
                                ...
                                print(f"Merged data from {temp_file} into {main_file}")
                        except (json.JSONDecodeError, KeyError) as e:
                            print(f"Could not merge data from {temp_file}: {e}")
        ...
        try:
            ...
            if temp_filename and os.path.exists(temp_filename):
                try:
                    ...
                    print(f"Cleaned up temp file {temp_filename} after error")
    ...
    def close(self):
        ...
        if self.data:  # Save any remaining data
            ...
            try:
                ...
                if success:
                    self.data = {}
                    print(f"Final log saved successfully: {self.log_file}")
                else:
                    print(f"Warning: Failed to save final log data")
            except Exception as e:
...
class PoseLogger(BaseLogger):
    ...
    def log_frame(self, init_hand_pose, init_robot_pose, current_hand_pose, transformed_pose):
        ...
        # print(f"Logging frame {self.frame_count} for {self.prefix}")
